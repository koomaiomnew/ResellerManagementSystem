...
const OrderTracking = () => {
  ...
  // ฟังก์ชันคำนวณเปอร์เซ็นต์ของหลอด Progress ให้พอดีเป๊ะ
  const getProgressWidth = () => {
    if (currentStepIdx === 0) return '0%';
    if (currentStepIdx === 1) return '50%';
    if (currentStepIdx === 2) return '100%';
    return '0%';
  };
      ...
      <div className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 -mt-6 pb-20 relative z-10">
        {order && (
          <div className={`transition-all duration-500 ${showResult ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-4'}`}>
            <div className="bg-white/80 backdrop-blur-xl rounded-[2rem] shadow-2xl shadow-slate-200/50 border border-black/60 overflow-hidden">
              ...
              {/* Progress Steps 🛠️ (ปรับการจัดวางเส้นใหม่ให้ไม่เกินขอบ) */}
              <div className="p-6 sm:p-10">
                <div className="relative mb-14 mt-4 max-w-xl mx-auto">
                  
                  {/* กรอบของเส้น (Track Wrapper): บีบ left/right ให้ตรงกับศูนย์กลางวงกลมพอดี */}
                  {/* วงกลมขนาด w-12 (48px) ครึ่งนึงคือ 24px (left-6) */}
                  ...
                  <div className="relative z-10 flex justify-between">
                    {steps.map((step, idx) => (
                      <div key={step.key} className="flex flex-col items-center relative">
                        ...
                        {/* ปรับข้อความให้อยู่ตรงกลางแบบ absolute เพื่อไม่ให้ดันโครงสร้างกล่องเสีย */}
                        <span className={`absolute top-14 left-1/2 -translate-x-1/2 w-32 mt-2 text-xs sm:text-sm font-semibold text-center transition-colors ${
              ...
              {/* Order Details */}
              <div className="px-6 sm:px-10 pb-8 mt-6">
